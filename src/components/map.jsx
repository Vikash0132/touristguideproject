...
const Map = ({ destination, startingCoordinates }) => {
  ...
  useEffect(() => {
    ...
    map.current = new maptilersdk.Map({
      ...
      center: userLocation || [77.209, 28.6139], // Default to Delhi if no user location
      zoom: zoom,
      ...
      if (destination?.coordinates) {
        ...
        map.current.addSource("route", {
          ...
          data: {
            ...
            geometry: {
              type: "LineString",
              coordinates: [userLocation, destination.coordinates],
            },
          },
        });
        ...
        map.current.fitBounds([userLocation, destination.coordinates], {
          padding: 50,
        });
      }
    });
    ...
    // Remove the setCenter call from the moveend event handler
  ...
  const distance = destination?.coordinates
    ? calculateDistance(userLocation, destination.coordinates)
    : null;
  ...
  return (
    <div className="map-wrap">
      ...
      {distance && (
        <div className="distance-info">Distance: {distance.toFixed(2)} km</div>
      )}
    </div>
  );
};
